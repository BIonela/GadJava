...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < studioDatabase.length; i++) {
            studioMoreThanTwoMovies(studioDatabase[i]);
        }

        for (int j = 0; j < studioDatabase.length; j++) {
            playsTheActor(studioDatabase[j]);
        }

        for (int k = 0; k < studioDatabase.length; k++) {
            actorAgeAbove(studioDatabase[k]);
        }
    }

    public static void studioMoreThanTwoMovies(Studio studio) {
        if (studio.getFilme().length > 2) {
            System.out.println(studio.getNume());
        }
    }

    public static void playsTheActor(Studio studio) {
        Film[] film = studio.getFilme();
        for (int i = 0; i < film.length; i++) {
            //System.out.println(film[i].getNume());
            Actor[] actor = film[i].getActori();
            for (int j = 0; j < actor.length; j++) {
                //System.out.println(actor[j].getNume());
                if (actor[j].getNume().equals("actor cu 2 oscaruri")) {
                    System.out.println(studio.getNume());
                }
            }
        }
    ...
    public static void actorAgeAbove(Studio studio) {
        Film[] film = studio.getFilme();
        for(int i = 0; i < film.length; i++) {
            Actor[] actor = film[i].getActori();
            for(int j = 0; j < actor.length; j++) {
                if(actor[j].getVarsta() > 50) {
                    System.out.println(film[i].getNume());
                    break;
                }
            }
